...
// ****** Pong Game ******
//
// Created by Aleksandr Medvedev on 12.06.2022
...
void screenRender(const int *rocket_1_Y, const int *rocket_2_Y,
                  const int *ball1_x, const int *ball1_y, int *scPlayer1,
                  int *scPlayer2i);
...
int main() {
  ...
  while (1) {
    screenRender(&rocket_1_Y, &rocket_2_Y, &ball_X, &ball_Y, &scorePlayer_1,
                 &scorePlayer_2);
    ...
    clearScreen();
    if (checkScore(&scorePlayer_1, &scorePlayer_2, &ball_X, &ball_Y)) {
      break;
    }
  }
  return 0;
...
void moveBall(int *ball1_x, int *ball1_y, int *vector_X, int *vector_Y,
  ...
  if (*ball1_x == 3 &&
      // Сравниваем положение ракетки 1 и мяча, чтобы в случае столкновения
      // менялся
  ...
  if (*ball1_x == 78 &&
      // Сравниваем положение ракетки 2 и мяча, чтобы в случае столкновения
      // менялся
...
void screenRender(const int *rocket_1_Y, const int *rocket_2_Y,
  ...
  for (int y = 0; y < 5; y++) {
    ...
    for (int x = 0; x < 82; x++) {
      ...
      if ((y == 0 && x != 0 && x != 81) ||
          // Условие помогает сделать отрисовку y = 0 символами # исключая 0 и
          // 81 ячейку по x
          (y == 4 && x != 0 && x != 81)) {
  ...
  for (int y = 0; y < 27; ++y) {
    ...
    for (int x = 0; x < 82; ++x) {
      ...
      if ((x == 2 && (y == *rocket_1_Y - 1 || y == *rocket_1_Y ||
                      y == *rocket_1_Y + 1)) ||
          // Условие считывает координату ракетки №1 игрока от 1 до 3, так как
          // ракетка занимает 3 символа
          (x == 79 && (y == *rocket_2_Y - 1 || y == *rocket_2_Y ||
                       y == *rocket_2_Y + 1))) {
        // Условие считывает координату ракетки №2 игрока от 1 до 3, так как
        // ракетка занимает 3 символа
        printf("|");
        ...
        // Прорисовка мяча символом o
                 ...
                 // Отрисовка верхней и нижней границы символом - на y = 0 и y =
                 // 26
                 (y == 26 && x != 0 && x != 81)) {
        printf("-");
      } else if ((x == 0 && y != 0 && y != 26) ||
                 // Отрисовка левой, правой и центральной границ символом | на x
                 // = 0 и x = 81 и x = 41
                 (x == 81 && y != 0 && y != 26) || (x == 82 / 2)) {
        printf("|");
      } else {
        printf(" ");
      }
    ...
    for (int x = 0; x < 82; x++) {
      ...
      if ((y == 0 && x != 0 && x != 81) ||
        ...
        printf("#");
                  ...
                  // Отрисовка левой границы символом | по x = 1 исключая y = 2
                  // и y = 0
                  (x == 81 && y != 0 && y != 2))) {
        // Отрисовка левой границы символом | по x = 81 исключая y = 2 и y = 0
        printf("|");
      } else if (x == 10) {
        // Указание адреса, где необходимо реализовать надпись инструкции по
        // управлению и счет игроков
        printf(
            "Нажми \'c\' чтобы выйти           Первый игрок: %d | Второй "
            "игрок: %d",
            *scPlayer1, *scPlayer2);
      } else if ((*scPlayer1 >= 10 && *scPlayer1 <= 21) ||
                 (*scPlayer2 >= 10 && *scPlayer2 <= 21)) {
        // Считывание счета игроков и смещение отрисовки правого поля в
        // зависимости от счета
      ...
      } else if ((*scPlayer1 >= 0 && *scPlayer1 < 10) ||
                 (*scPlayer2 >= 0 && *scPlayer2 < 10)) {
        if ((y == 1 && x < 16) || (y != 1)) {
          printf(" ");
        }
...
int checkScore(int *scPlayer1, int *scPlayer2, int *ball1_x, int *ball1_y) {
  ...
  if (*scPlayer1 == 21) {
    printf("\nОтлично сработано!!! Выиграл игрок 1 со счётом %d : %d",
           *scPlayer1, *scPlayer2);
    // Выводит сообщение о победе в консоль
    ret = 1;
  } else if (*scPlayer2 == 21) {
    printf("\nОтлично сработано!!! Выиграл игрок 2 со счётом %d : %d",
           *scPlayer2, *scPlayer1);
    ret = 1;
  }
  return ret;
...
void clearScreen() {
  // Функция очищает экран от предыдущих отрисовок, что позволяет нам видеть
  // только 1 игровую таблицу
  system("printf \033c");
}
...
void moveRocket(int *rocket_1_Y, int *rocket_2_Y) {
  ...
  if (kbnit()) {
    ...
    if (temp == 'a' && ((*rocket_1_Y - 2) != 0))
      // Условия для игрока_1. Если нажата клавиша А и ракетка находится не в
      // верхней границе
      *rocket_1_Y -= 1;
    // то двигаем ее наверх
      ...
      *rocket_1_Y += 1;
      ...
      // Условия для игрока_2. Точно та же логика, что и для игрока_1, только с
      // кнопками
      *rocket_2_Y -= 1;
    // К и М
